...
`masstools` is a lightweight, dependency-conscious R package that provides core utilities for mass-spectrometry and metabolomics workflows. As part of the [`tidymass` ecosystem](https://www.tidymass.org/), it complements Bioconductor infrastructure (e.g., `Spectra`, `MSnbase`) by offering optimized, reusable building blocks for routine tasks such as data parsing, spectral similarity, and identifier conversion—helping bridge the gap between low-level data structures and applied analyses.

**Key functionality includes:**

-   **Spectral similarity & MS2 matching:** dot product and related metrics for tandem MS comparisons.
-   **Chemical utilities:** helpers for formula/identifier processing and conversions.
-   **I/O helpers:** convenient tools for reading, inspecting, and manipulating MS data.
-   **Visualization aids:** simple plotting utilities for spectra and related results.

Developed by Xiaotao Shen and contributors, `masstools` is designed to slot into modern R workflows and integrate smoothly with other `tidymass` and Bioconductor packages. Its focus on small, stable utilities and minimal dependencies makes it a reliable foundation for reproducible mass-spectrometry analyses in R.
...
MS2 plot and MS2 matching plot. The `ms2_plot()` function is designed to visualize one or two MS2 spectra, enabling side-by-side comparison. This is useful in spectral library matching, experimental result inspection, or quality control.
...
We can use the `trans_id_database()` to get the databases that cts.fiehnlab. **Caution**: `trans_id_database` function temporarily can not be used.
